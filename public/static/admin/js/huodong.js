...
//创建一个上传组件
  upload.render({
    ...
    ,done: function(res, index, upload){ //上传后的回调
    	if(res.error == 0)
		{
    		...
    		$('input[name="activity_logo"]').val(res.data_url);
		}else{
			layer.msg(res.msg,{
				icon:5
			});
  ...
  //}
    ...
    ,toolbar:"#toolbarDemo"
    ,defaultToolbar:['filter', 'print', 'exports']//是否显示[],不显示就直接[]
    ,totalRow: false //开启合计行
    ,cols: [[ //表头
        {type: 'checkbox',width:"5%",field:"id",fixed: 'left'}
       ,{field: 'activity_name',width: '25%',  title: '活动名',align:'center'}
       ...
       ,{field: 'status',width: '10%',  title: '状态',align:'center',templet:'#sex'}
	  			  ...
	  			  if(ids)
	  			{
			  	  ...
			  	  layer.confirm('真的删除行么', function(index){
			      	...
			      	$.ajax({
			      		url:'del',
			      		type:"post",
			      		data:{id:ids,page:cur,limit:limit},
			      		dataType:"json",
			      		success:function(r){
			      			if(r.code == 1){
...
//监听行工具事件
  ...
  form.on('submit(formDemos)',function(data){
	   let param = {
			 activity_name:data.field.username,
	  };
	  tableins.reload({
		  where:param
	  });
	  ...
	  $.ajax({
		  ...
		  success:function(res)
		  {
			  if(res.code == 1)
			  {
				  ...
				  setTimeout(function(){
					  window.location.href="/admin/Huodong/index";
				  },2000);
			  }else{
				  layer.msg(res.msg,{
					  icon:5
